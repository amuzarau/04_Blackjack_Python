...
def main():
    ...
    while True:   # Main game loop
        ...
        #Give the dealer and player tow cards from the ech each:
        deck = get_deck()
...
def get_bet(max_bet):
    ...
    while True: # # Keep asking until they enter a valid amount.
        ...
        bet = int(bet)
        if 1 <= bet <= max_bet:
            return bet  # Player entered a valid bet.isdecimal
...
def get_hand_value(cards):
    ...
    for i in range(number_of_aces):
        # If another 10 can be aded with busting, do so:
        if value + 10 <= 21:
            value += 10
...
def display_cards(cards):
    ...
    rows = ['','','','','']  # Print the top line of the card.
